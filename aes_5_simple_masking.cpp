...
array<uint8_t, 16> subBytes(const array<uint8_t, 16> &text)
{
	...
	uint8_t m_in  = static_cast<uint8_t>(rand() % 256);
	...
	for (size_t i = 0; i < 16; ++i)
	{
		uint8_t masked_in = text[i] ^ m_in;

		uint8_t masked_out = maskedSBox[masked_in];

	
		output[i] = masked_out ^ m_out;
	}
...
array<uint8_t, 16> AES128(const array<uint8_t, 16> &plainText, const array<uint8_t, 16> &initialKey)
{
	...
	array<uint8_t, 16> state = addRoundKey(plainText, roundKeys[0]);
